...
def interaction_morph(
    ...
    @wraps(func)
    async def f(_: discord.ui.Button, interaction: discord.Interaction) -> None:
...
class LeagueOfNations(commands.AutoShardedBot):
    ...
    async def get_user_from_id_lookup(self, interaction: qalib.QalibInteraction[Any], user_id: int, func: Callable[Concatenate[Nation, P], Coroutine[None, None, None]], *args: P.args, **kwargs: P.kwargs):
        ...
        await interaction.display("nation_preview", keywords={"nation": nation, "require_mechanisms": True}, callables={"on_accept": on_accept, "on_reject": on_reject})


    async def get_user_from_nation_lookup(self, interaction: qalib.QalibInteraction[Any], nation_name: str, func: Callable[Concatenate[Nation, P], Coroutine[None, None, None]], *args: P.args, **kwargs: P.kwargs):
        ...
        async def on_select(item: discord.ui.Select, new_interaction: discord.Interaction):
            ...
            async def on_reject(_: discord.ui.Button, i: discord.Interaction):
                await i.response.defer() 
                await interaction.display("nation_lookup", keywords={"nations": nations, "require_mechanisms": True}, callables={"on_select": on_select})
